...
__all__ = ['ADXTrader', 'HODLTrader', 'PercentageTrader', 'SMATrader']
...
def reweight(context,data,wt,min_pct_diff=0.1):
    ...
    orders = {}
    pct_diff = 0
    for security in wt.keys():
        target = liquidity*wt[security]/data[security].price
    ...
    if pct_diff > min_pct_diff:
        ...
        for security in orders.keys(): order(security, orders[security])
...
def handle_data(context, data):
    ...
    wt = dict(((security,context.weight if security in top_securities and abs_mom(security) > 0 else 0.0) for security in context.securities))
