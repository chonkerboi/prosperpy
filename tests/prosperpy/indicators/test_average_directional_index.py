...
import prosperpy
import prosperpy.indicators.average_directional_index
...
class TestAverageDirectionalMovement(unittest.TestCase):
    def test_raise_not_implemented_error(self):
        with self.assertRaises(NotImplementedError):
            prosperpy.indicators.AverageDirectionalMovement(get_candles())
...
class TestMinusDirectionalMovement(unittest.TestCase):
    def test_directional_movement(self):
        ...
        minus_dm = prosperpy.indicators.MinusDirectionalMovement(candles[1:period+1])
        self.assertEqual(minus_dm.period, period)
    ...
    def test_directional_indicator(self):
        ...
        true_range = prosperpy.indicators.average_directional_index.TrueRange(candles[1:period+1])
        minus_dm = prosperpy.indicators.MinusDirectionalMovement(candles[1:period+1], true_range=true_range)
...
class TestPlusDirectionalMovement(unittest.TestCase):
    def test_directional_movement(self):
        ...
        plus_dm = prosperpy.indicators.PlusDirectionalMovement(candles[1:period+1])
        self.assertEqual(plus_dm.period, period)
    ...
    def test_directional_indicator(self):
        ...
        true_range = prosperpy.indicators.average_directional_index.TrueRange(candles[1:period+1])
        plus_dm = prosperpy.indicators.PlusDirectionalMovement(candles[1:period+1], true_range=true_range)
...
class TestTrueRange(unittest.TestCase):
    def test_true_range(self):
        ...
        true_range = prosperpy.indicators.average_directional_index.TrueRange(candles[1:period+1])
        self.assertEqual(true_range.period, period)
...
class TestDirectionalIndex(unittest.TestCase):
    @unittest.mock.patch('prosperpy.indicators.MinusDirectionalMovement.add')
    @unittest.mock.patch('prosperpy.indicators.PlusDirectionalMovement.add')
    @unittest.mock.patch('prosperpy.indicators.average_directional_index.TrueRange.add')
    def test_add(self, mock_true_range_add, mock_plus_directional_movement, mock_minus_directional_movement):
        period = 14
        candles = get_candles()
        index = prosperpy.indicators.DirectionalIndex(candles[1:period+1])
...
class TestAverageDirectionalIndex(unittest.TestCase):
    def test_average_directional_index(self):
        period = 28
        candles = get_candles()
        data = ['30.10818270237424', '28.52243274412106', '27.04995064002883', '25.91925250858815', '24.08469727139116',
                '22.91288154386045', '22.1873379482591', '21.63217119448856', '20.89122517232141', '19.66519162377017',
                '18.76859395434995', '18.79172038189738', '18.85948727478337']
        data = [decimal.Decimal(item) for item in data]

        adx = prosperpy.indicators.AverageDirectionalIndex(candles[1:period+1])
        self.assertEqual(adx.period, 14)
        self.assertEqual(adx.value, decimal.Decimal('32.35342170720747'))
